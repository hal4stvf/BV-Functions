...
%
%and, by the estimates already found, we can conclude that, up to choosing a suitable $\delta(\epsilon)$, it is all bounded by $C \epsilon$ for $n$ big enough, thus proving weak-star convergence of measures. 
...
\begin{example} \label{ex:Giusti_cheese} 
...
We list now some relevant applications of the coarea formula: the polar coordinates' change of variables and the case in which $f$ is the distance function from a compact set.
...
\begin{theorem} \label{convblowup} If $E$ is a set of finite perimeter in $\Omega$, $x \in \redb E$ and $\nu(x) = - \nu_{E}(x)$, then
...
%$\forall \eps > 0$ we have
%
%\[ \{y \in \mathbb{R}^{n} : |\chi_{E}(y) - 1| \ge \eps \} \cap B(x,r) = E^{0} \cap B(x, r) . \]
%
%So, 
%\begin{align*} \lim_{r \to 0} \frac{|\{y \in \mathbb{R}^{n} : |\chi_{E}(y) - 1| \ge \eps \} \cap B(x,r)|}{|B(x, r)|} &= \lim_{r \to 0} \frac{|E^{0} \cap B(x, r)|}{|B(x, r)|} \\
%&= 1 - D(E, x)  = 0. 
%\end{align*}
%
%Therefore, $\forall a \in \mathbb{R}^{n}$
%\begin{align*} & \frac{ |\{y \in \mathbb{R}^{n} : |\chi_{E}(y) - 1| \ge \eps \} \cap B(x, r) \cap \Pi_{a}(x)|}{|B(x, r) \cap \Pi_{a}(x)|} \\
%& \le \frac{|\{y \in \mathbb{R}^{n} : |\chi_{E}(y) - 1| \ge \eps \} \cap B(x, r)|}{|B(x, r)|} \frac{|B(x, r)|}{|B(x, r) \cap \Pi_{a}(x)|} \\
%& = 2 \frac{|\{y \in \mathbb{R}^{n} : |\chi_{E}(y) - 1| \ge \eps \} \cap B(x, r)|}{|B(x, r)|} \to 0 \ \ \text{as} \ \ r \to 0.
%\end{align*}
%
